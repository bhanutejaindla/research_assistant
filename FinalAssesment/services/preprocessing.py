...
SKIP_DIRS = {"git", "node_modules", "__pycache__", "venv", ".idea", "usr", "code"}
    ...
    for root, dirs, files in os.walk(repo_path):
        ...
        for file in files:
            ...
            if file.lower() in ["main.py", "app.py", "index.js"]:
                ...
                await event_manager.send(project_id, f"Entry point detected: {file}")
            ...
            # Extract Python function/class chunks
            if file.endswith(".py"):
...
async def parse_dependencies(file_path: str) -> List[str]:
    """Parse dependencies from common dependency files."""
    deps = []
    try:
